...
def get_dtws_cluster(data, method, metric, max_cluster, cluster_select):
    cwd = os.getcwd()
    path = cwd+'/dtws_'+str(method)+'_'+str(metric)+'_'+str(cluster_select)+'.pkl'
...
def get_dtw_uniq_cluster(df_res, df_raw, method, metric, max_cluster, cluster_select):
    cmps = df_res['components'][int(max_cluster)-int(cluster_select)]
    res = rep_string(cmps).split(' ')
    df_raw = df_raw.T
    df_raw.columns = list(map(str,df_raw.columns))
    ...
    res = get_dtws_cluster(df_all_res, method, metric, max_cluster, cluster_select)
    ...
    plt.savefig('data_out/dtw_uniq_cluster_draw_'+str(method)+'_'+str(metric)+'_'+str(cluster_select)+'.pdf')
    plt.savefig('data_out/dtw_uniq_cluster_draw_'+str(method)+'_'+str(metric)+'_'+str(cluster_select)+'.png')
...
def matrix_plot(df,category):
    ...
    plt.savefig('data_out/matrix_plot_'+str(category.lower())+'.pdf')
    plt.savefig('data_out/matrix_plot_'+str(category.lower())+'.png')
